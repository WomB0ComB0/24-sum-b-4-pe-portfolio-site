...
check_mysql() {
    ...
    mysql -u root -p"${MYSQL_ROOT_PASSWORD}" -e "GRANT ALL PRIVILEGES ON ${TEST_MYSQL_DATABASE}.* TO '${MYSQL_USER}'@'localhost';"
    ...
    # Create tables
    mysql -u root -p"${MYSQL_ROOT_PASSWORD}" -D ${MYSQL_DATABASE} -e "CREATE TABLE IF NOT EXISTS hobbies (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), description TEXT, image VARCHAR(255), created_at DATETIME, updated_at DATETIME);"
    mysql -u root -p"${MYSQL_ROOT_PASSWORD}" -D ${MYSQL_DATABASE} -e "CREATE TABLE IF NOT EXISTS projects (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), description TEXT, url VARCHAR(255), language VARCHAR(255), created_at DATETIME, updated_at DATETIME);"
    mysql -u root -p"${MYSQL_ROOT_PASSWORD}" -D ${MYSQL_DATABASE} -e "CREATE TABLE IF NOT EXISTS timeline (id INT AUTO_INCREMENT PRIMARY KEY, title VARCHAR(255), description TEXT, date DATE, created_at DATETIME, updated_at DATETIME);"
}
...
run_tests() {
    mysql -h 127.0.0.1 -uroot -p"${MYSQL_ROOT_PASSWORD}" -e "CREATE DATABASE IF NOT EXISTS ${TEST_MYSQL_DATABASE};" || echo "Failed to create database ${TEST_MYSQL_DATABASE}"
    mysql -h 127.0.0.1 -uroot -p"${MYSQL_ROOT_PASSWORD}" -e "USE ${TEST_MYSQL_DATABASE};" || echo "Failed to use database ${TEST_MYSQL_DATABASE}"
    output=$(python -m unittest discover -s tests/unit -p "test_*.py" 2>&1)
    exit_code=$?
    mysql -h 127.0.0.1 -uroot -p"${MYSQL_ROOT_PASSWORD}" -e "DROP DATABASE IF EXISTS ${TEST_MYSQL_DATABASE};" || echo "Failed to drop database ${TEST_MYSQL_DATABASE}"
